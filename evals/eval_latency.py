...
DELAY_BETWEEN_CALLS = 5  # Sleep in seconds between calls to respect Gemini 15 RPM limit
...
def benchmark(pipeline):
    ...
    for q_idx, question in enumerate(QUESTIONS):
        for rep in range(REPEATS):
            sample_counter += 1
            print(f"  Sample {sample_counter}/{total_samples} (Question {q_idx + 1}, Repeat {rep + 1})...")

            # Pause before calling the LLM to avoid 429 quota exhaustion
            time.sleep(DELAY_BETWEEN_CALLS)
