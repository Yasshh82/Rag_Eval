...
# --- Pricing: gemini-3.1-flash-lite, USD per 1M tokens (verified Aug 2026). ---
...
PRICE_INPUT_PER_1M        = 0.25    # cache-miss input
PRICE_CACHED_INPUT_PER_1M = 0.025   # cached (repeated prefix) input -- half price
PRICE_OUTPUT_PER_1M       = 1.50    # output (4x input -- long answers dominate)
...
COST_BUDGET_PER_QUERY_USD = 0.0014  # e.g. must stay under ~0.13 INR / query
...
def benchmark(pipeline):
    ...
    for q_idx, question in enumerate(QUESTIONS):
        for rep in range(REPEATS):
            sample_counter += 1
            print(f"  Measuring sample {sample_counter}/{total_samples} (Question {q_idx + 1}, Repeat {rep + 1})...")
...
def report(rows):
    ...
    print("\n" + "=" * 70)
    print(f"COST  (gemini-3.1-flash-lite @ ${PRICE_INPUT_PER_1M}/${PRICE_OUTPUT_PER_1M} per 1M in/out)")
    print("=" * 70)
    print(f"samples                : {n}")
