...
""".strip(),
    ...
    result = evaluate(test_cases=test_cases, metrics=[SCOPE_METRIC], async_config=AsyncConfig(run_async=False))
...
def run_leakage(rag, verbose=True):
    ...
    protected_result = evaluate(test_cases=content_test_cases, metrics=[PROTECTED_LEAKAGE_METRIC], async_config=AsyncConfig(run_async=False))
    pii_result       = evaluate(test_cases=pii_test_cases, metrics=[PII_LEAKAGE_METRIC], async_config=AsyncConfig(run_async=False))
...
def run_toxicity(rag, verbose=True):
    ...
    result = evaluate(test_cases=test_cases, metrics=[TOXICITY_METRIC], async_config=AsyncConfig(run_async=False))
    s = summarize_results(result)
    if verbose:
